...
LOG_FILE="$REPO_PATH/logs/installation_ubuntu-kubectl_$TIMESTAMP.log"
...
cd "$REPO_PATH/ansible/ubuntu-kubectl-ansible" || { echo "Error: 'ubuntu-kubectl-ansible' directory not found in $REPO_PATH"; exit 1; }

# Run the Ansible playbook with the inventory file and ask for vault password
ansible-playbook playbooks/deploy-kubectl-pod.yml -i ../../configs/inventory.yml --ask-vault-pass
echo "ubuntu-kubectl installation complete!"

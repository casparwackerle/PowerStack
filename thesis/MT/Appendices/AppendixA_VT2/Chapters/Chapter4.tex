\chapter{Approaches and Tools for Container Energy Measurement}
\label{vt2_Chapter4}

\section{Introduction}
\label{vt2_sec:tool-intro}
...
\label{vt2_sec:non-k8s-tools}
\subsection{Server-Level Energy Monitoring}
\label{vt2_sec:server-tools}
...
\begin{itemize}
    ...
    \item \textbf{SmartWatts-formula}\parencite{fieni2020smartwatts}: A power model implementation (in Python) using HWPC data to estimate power consumption dynamically. It employs online linear regression provided by the Python \textit{scikit-learn}\parencite{scikit-learn} library, enabling accurate runtime learning of workload-specific power signatures. SmartWatts is further detailed in section~\ref{vt2_sec:smartwatts}.
...
\subsection{Kepler}
\label{vt2_sec:kepler}

\subsubsection{Overview and Goals}
\label{vt2_sec:kepler-overview}
...
Tables~\ref{vt2_tab:kepler-metric-inputs} and~\ref{vt2_tab:kepler-metadata-inputs} summarize the distinct types of input Kepler collects. Metric inputs provide raw resource and energy data, while metadata inputs allow this data to be mapped to containers, VMs, or system processes.
...
\begin{table}[h]
    ...
    \label{vt2_tab:kepler-metric-inputs}
\end{table}
...
\begin{table}[h]
    ...
    \label{vt2_tab:kepler-metadata-inputs}
\end{table}
...
\label{vt2_sec:kepler-attribution-timing}
...
While the attribution issues demonstrated by Pijnacker et al.\ are the most critical and empirically validated shortcomings of Kepler, a detailed inspection of the source code conducted as part of this thesis (see Section~\ref{vt2_sec:kepler-attribution}) confirms these problems and reveals several additional implementation and documentation-related concerns. Although these issues are less severe in their impact, they further highlight the current limitations of Kepler as a mature and reliable observability tool.
...
At the power measurement layer, Kepler continues to rely on RAPL readings, now referred to as ``energy zones''. In addition to standard \code{package}, \code{core}, \code{uncore}, and \code{dram} zones, Kepler v0.10.0 dynamically detects the \code{psys} domain, expanding applicability to non-server-grade processors. Multi-socket systems are supported via automatic aggregation of identically named RAPL zones, with counter wraparounds correctly handled through delta calculations based on maximum energy values.
...
\label{vt2_sec:scaphandre}

\subsubsection{Overview and Goals}
\label{vt2_sec:scaphandre-overview}
...
\subsubsection{Architecture and Metric Sources}
\label{vt2_sec:scaphandre-architecture}
...
The \textit{sensors} abstraction layer also applies key data preprocessing steps, which are discussed in section~\ref{vt2_sec:scaphandre-attribution}.
...
\label{vt2_sec:scaphandre-attribution}
...
\subsubsection{Validation and Research Context}
\label{vt2_sec:scaphandre-validation}
...
\subsubsection{Limitations and Open Issues}
\label{vt2_sec:scaphandre-limitations}
...
\label{vt2_sec:smartwatts}
\subsubsection{Overview and Goals}
\label{vt2_sec:smartwatts-overview}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.7\textwidth]{Appendices/AppendixA_VT2/Figures/smartwatts_architecture.png}
    \caption[SmartWatts architecture]{SmartWatts Architecture}
    \label{vt2_fig:smartwatts_architecture}
\end{figure}

\subsubsection{Attribution Model}
\label{vt2_sec:smartwatts-attribution}
...
\subsubsection{Validation and Research Context}
\label{vt2_sec:smartwatts-validation}
...
\subsubsection{Limitations and Open Issues}
\label{vt2_sec:smartwatts-limitations}

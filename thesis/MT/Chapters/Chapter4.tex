...
% WHAT is computed and WHY (not HOW).
% Per-node, per-window reasoning.
% High-level statement of outputs: windowed node and workload attributions.

\subsection{Attribution Goals and Non-Goals}
% Goals: accuracy-first, conservation, temporal consistency.
% Non-goals: instantaneous attribution, cross-node attribution, fairness, exporter concerns.

\subsection{Analysis Engine as an Architectural Actor}
% Define the analysis engine as the component that triggers per-window analysis,
% applies the attribution pipeline, and produces derived metrics.

\subsection{Attribution Windows and Timebase Assumptions}
\label{subsec:attrib_windows}
% Event-time perspective and monotonic timebase assumption.
% Define attribution windows conceptually and motivate windowed attribution.

\subsection{Stage 1: Component Metric Construction}
\label{subsec:stage1_component_metrics}
% Define the derived per-window component metrics that serve as inputs to attribution stages.
% Present domains in canonical order BPF -> RAPL -> Redfish -> GPU.

\subsubsection{eBPF Utilization Metrics (Totals and Aggregates)}
% Architectural definition of utilization signals.
% Distinguish node-level totals from workload-aggregated utilization.

\subsubsection{RAPL Domain Energy Metrics}
% Architectural definition of per-domain energy over a window (package, core, DRAM, uncore).
% Treat domains as independent energy sources.

\subsubsection{Redfish-Corrected System Energy Metric}
% Define redfish_corrected as a fused, authoritative system energy metric.

\subsubsection{GPU-Corrected Energy Metric}
% Define gpu_corrected as an intra-domain fusion producing a higher-quality GPU energy signal.

\subsection{Stage 2: System-Level Energy Model and Residual}
\label{subsec:stage2_residual}
% Define the conservation relationship and residual energy as a first-class quantity.

\subsubsection{Global Energy Decomposition}
% sum(RAPL domains) + sum(gpu\_corrected) + Residual = redfish\_corrected
% Per-node, per-window.

\subsubsection{Residual Definition and Interpretation Limits}
% Residual as unassigned remainder; limits of interpretation.

\subsubsection{Negative Residuals as a Temporal Consequence}
% Accepted architectural consequence due to lag/delay and response dynamics.

\subsubsection{Conservation and Consistency Constraints}
% Precisely stated constraints: conservation, no double counting, scope constraints.
% Clarify what may be negative and what must not.

\subsection{Stage 3: Idle and Dynamic Energy Semantics}
\label{subsec:stage3_idle_dynamic}
% Define idle vs dynamic energy semantics for energy domains.
% Present in canonical order where applicable (RAPL -> Redfish -> GPU).
% (BPF has no energy split.)

\subsubsection{RAPL Idle and Dynamic Semantics}
% Define idle/dynamic for each RAPL domain.

\subsubsection{Redfish-Corrected Idle and Dynamic Semantics}
% Define whether and how redfish\_corrected participates in idle/dynamic interpretation.

\subsubsection{GPU Idle and Dynamic Semantics}
% Define dynamic GPU attribution basis; state GPU idle handling policy explicitly.

\subsection{Stage 4: Workload Attribution and Aggregation}
\label{subsec:stage4_workload_attrib}
% Define how node-level quantities are allocated to workloads.
% Centralize all hierarchy and mapping concepts here to avoid redundancy.

\subsubsection{Attribution Identity and Join Contract}
% Architectural requirement: stable mapping between process/cgroup activity and Kubernetes objects.
% Validity conditions and degradation conceptually when identity is missing.

\subsubsection{CPU Dynamic Attribution to Workloads}
% Conceptual proportional attribution driven by eBPF utilization signals.

\subsubsection{CPU Idle Energy Allocation to Workloads}
% Architectural rule for allocating idle CPU energy to workloads.

\subsubsection{GPU Dynamic Attribution to Workloads}
% Conceptual allocation using gpu\_corrected and GPU utilization signals.

\subsubsection{GPU Idle Attribution to Workloads}
% Conceptual allocation using gpu\_corrected and GPU utilization signals.

\subsubsection{Workload-Level Utilization and Energy Aggregates}
% Define which aggregates are produced at workload/pod/container levels and their semantics.

\subsection{Stability, Validity Conditions, and Architectural Limits}
\label{subsec:analysis_limits}
% Temporal stability expectations, assumptions, and explicit non-guarantees.

\subsection{Architectural Consequences}
% What this enables for implementation and evaluation; no exporter details.

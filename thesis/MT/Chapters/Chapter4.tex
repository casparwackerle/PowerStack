...
For the corrected construction, system power is reconstructed on a uniform grid of bins $k$ with width $\Delta$ seconds over a finite horizon.
...
However, the only available system-wide signal, Redfish-reported power, exhibits limited and variable temporal fidelity.
This creates a fundamental tension between physical conservation and temporal alignment that cannot be resolved by algebraic refinement alone.

\paragraph{Conceptual Model}
Stage~2 introduces a system-level energy balance scoped to a single node and a single analysis window.
Total system energy is taken from the Redfish-integrated signal, while accounted component energy is defined as the sum of all modeled contributors.
Any remaining energy is captured explicitly as a residual term, which represents unmodeled components and temporal mismatch rather than error.
...
Let $E_{\text{parts}}$ denote the sum of all accounted component energies for the same window.

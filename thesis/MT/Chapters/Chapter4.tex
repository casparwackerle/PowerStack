...
% ----------------------------------------------------------------------
\subsection{GPU Subsystem Architecture}
...
A central architectural design choice is the use of high-frequency instantaneous
power fields exposed through NVIDIA's field interfaces, rather than the
traditional one-second trailing average returned by \code{nvmlDeviceGetPowerUsage}.
This enables sub-second attribution and allows Tycho to track energy behaviour
with temporal fidelity comparable to RAPL.

\paragraph{Backend Abstraction}
The collector interfaces with NVIDIA hardware through a backend layer that
supports both \code{NVML} and \code{DCGM}.  This abstraction performs device
enumeration, capability probing, and topology discovery, including the presence
of MIG instances.  The architecture does not assume uniform support across
devices.  Cumulative energy counters, instantaneous power fields, and
process-level utilisation may or may not be available.  All capability
differences are managed through per-device feature masks inside the
implementation.

\paragraph{Conceptual Sampling Model.}
GPU drivers update power and utilisation metrics at discrete, hardware-defined
cadences that are not visible to callers.  Polling at a fixed interval risks
either missing updates or repeatedly reading identical values.  Tycho therefore
employs a phase-aware sampling model that infers the implicit publish rhythm
from observed updates and aligns sampling effort with predicted update edges.
...
In burst mode, the subsystem temporarily increases its sampling frequency when
...
hardware update and Tycho's observation of it.  The architecture guarantees that
sampling remains event-driven rather than periodic.
...
constraints remain.  
Backend capabilities vary significantly across NVIDIA devices.  
Some accelerators expose high-quality instantaneous power fields and cumulative
energy counters, while others expose only averaged power and coarse utilisation.
Process telemetry is inherently retrospective and cannot represent instantaneous
states.  

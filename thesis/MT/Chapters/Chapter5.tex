...
Pod entries are keyed by pod UID, container entries by normalized runtime container ID, and process entries by PID with the process start token (\texttt{StartJiffies}) retained for disambiguation.

PID reuse is handled by treating the pair (PID, \texttt{StartJiffies}) as the effective process identity.
...
Cycles are triggered externally; the engine does not self-schedule, maintain timers, or perform background work.
...
This section describes how the materialization model defined in \S~\ref{sec:arch_analysis_metric_materialization} is enforced at runtime.
...
The architectural definition of eBPF-based utilization metrics must be realized from discrete, per-tick kernel observations.
The implementation must construct window-aligned CPU time-share ratios and cumulative counters while preserving monotonicity, conservation, and interpretability under discretization and partial window overlap.

\paragraph{Conceptual Model}
Implementation operates on streams of eBPF ticks exposing per-tick deltas for CPU execution classes and kernel or hardware events.
For each analysis window, deltas are integrated over the effective window interval to produce window-scoped utilization quantities.
CPU time is normalized against node capacity to obtain ratios, while all other signals are accumulated into persistent node-level counters.

\paragraph{Formalization}
Discrete eBPF deltas are integrated over window boundaries using overlap-aware aggregation to approximate the continuous quantities defined architecturally.
CPU idle, interrupt, and soft-interrupt times are integrated independently and normalized by the product of window duration and logical CPU count.
Active CPU utilization is materialized as the complement of the non-active components, ensuring exact conservation of total CPU capacity.
Event-based signals are summed across all processes and accumulated over successive windows, yielding monotonic counters consistent with the architectural model.

\paragraph{Design Decisions}
Integration across window boundaries admits fractional contributions to preserve conservation in the presence of misaligned ticks.
Internal accumulation is therefore performed using floating-point state, while exported counters are materialized as integer-valued quantities.
Active CPU time is not measured directly but derived as a residual to avoid dependence on additional kernel signals and to guarantee ratio consistency.

\paragraph{Implementation Consequences}
The realized metrics uphold the architectural guarantees of strict monotonicity for counters and exact partitioning of CPU capacity across utilization classes.
Counters are scoped to the lifetime of the Tycho process and may reset on restart.
If insufficient samples are available to conservatively integrate a window, metric emission for that window is suppressed.
These behaviors ensure correctness under sampling variability while maintaining stable semantics for downstream consumption.
...
\begin{table}[h]
    \centering
    \begin{tabular}{p{4.0cm} p{2.0cm} p{1.8cm} p{4.8cm}}
    \toprule
    \textbf{Metric} & \textbf{Type} & \textbf{Unit} & \textbf{Labels} \\
    \midrule
    \multicolumn{4}{l}{\textit{CPU time-share ratios}} \\[4pt]
    \code{bpf\_cpu\_idle\_ratio} & Gauge & ratio & \code{source="ebpf"} \\
    \code{bpf\_cpu\_irq\_ratio} & Gauge & ratio & \code{source="ebpf"} \\
    \code{bpf\_cpu\_softirq\_ratio} & Gauge & ratio & \code{source="ebpf"} \\
    \code{bpf\_cpu\_active\_ratio} & Gauge & ratio & \code{source="ebpf"} \\[4pt]

    \multicolumn{4}{l}{\textit{Aggregated cumulative counters}} \\[4pt]
    \code{bpf\_cpu\_instructions} & Counter & count & \code{source="ebpf"} \\
    \code{bpf\_cpu\_cycles} & Counter & count & \code{source="ebpf"} \\
    \code{bpf\_cache\_misses} & Counter & count & \code{source="ebpf"} \\
    \code{bpf\_page\_cache\_hits} & Counter & count & \code{source="ebpf"} \\
    \code{bpf\_irq\_net\_tx} & Counter & count & \code{source="ebpf"} \\
    \code{bpf\_irq\_net\_rx} & Counter & count & \code{source="ebpf"} \\
    \code{bpf\_irq\_block} & Counter & count & \code{source="ebpf"} \\
    \bottomrule
    \end{tabular}
    \caption{Exported node-level utilization metrics derived from \code{eBPF} observations.}
...
\textbf{Metric} & \textbf{Type} & \textbf{Unit} & \textbf{Labels} \\
\midrule
\multicolumn{4}{l}{\textit{RAPL component totals (per domain; pkg, core, uncore, dram)}} \\
\midrule
\code{tycho\_energy\_mj} & Counter & mJ & \code{domain}, \code{kind="total"}, \code{source="rapl"} \\
\code{tycho\_power\_mw}  & Gauge   & mW & \code{domain}, \code{kind="total"}, \code{source="rapl"} \\
\bottomrule
\end{tabular}
...
\paragraph{Problem Statement}
GPU telemetry exposes multiple power- and energy-related observations that describe the same physical process under incompatible temporal semantics.
Instantaneous power samples, one-second averaged power values, and optional cumulative energy counters are delivered concurrently but cannot be combined by direct integration or signal selection without introducing temporal bias or discarding information.
...
The problem addressed here is therefore not measurement scarcity, but the absence of a principled method to reconcile redundant GPU observations into a single, temporally refined, internally consistent power and energy signal suitable for corrected-time analysis.

\paragraph{Conceptual Model}
GPU power is modeled as a latent, non-negative, continuous-time signal that is reconstructed on a uniform corrected-time grid and treated as the authoritative representation of device power.
All raw GPU observations are interpreted as constraints on this latent signal rather than as independent metrics.
Instantaneous and averaged power samples impose soft consistency constraints, while cumulative energy counters, when available, act as dominant anchors that strongly shape the reconstructed signal without enforcing exact equality.
Smoothness constraints encode physical plausibility by penalizing implausible curvature while preserving total energy.
The resulting reconstructed power series serves as the sole basis for deriving windowed GPU energy and power metrics, ensuring maximal use of available information while explicitly acknowledging the modeled nature of the signal.

\paragraph{Formalization}
For each GPU device, power is represented by a reconstructed sequence
\( p = \{p_k\}_{k=0}^{N-1} \), defined on a uniform corrected-time grid with spacing \( \Delta t \).
The reconstruction is posed as a constrained optimization problem in which \( p \) minimizes a weighted discrepancy to observed telemetry while satisfying physical plausibility.
...
The architectural objective is defined as
\begin{equation}
\min_{p \ge 0}
...
\end{equation}
where the second-difference term penalizes curvature and encodes smoothness without imposing a prior on absolute magnitude.
Non-negativity \( p_k \ge 0 \) enforces physical feasibility.
Cumulative energy observations contribute constraints whose weights dominate when present, ensuring strong energy consistency over time without enforcing hard equality.
...
\paragraph{Design Decisions}
Reconstruction by constrained fusion is chosen over signal selection to avoid privileging any single GPU telemetry source and to preserve all available information.
Cumulative energy counters are assigned dominant weight when available because they encode absolute energy change, yet they remain soft constraints to tolerate counter gaps, resets, and delayed reporting.
Instantaneous and averaged power samples are retained as complementary observations that improve temporal resolution and stabilize reconstruction in the absence of energy counters.
Smoothness is imposed via second differences to suppress implausible oscillations without biasing total energy or absolute power level.
No magnitude prior is introduced, as any bias toward lower power would conflict with the accuracy-first objective and distort sustained-load estimates.

\paragraph{Architectural Consequences}
This formulation yields a single corrected GPU power signal per device that is temporally finer than any raw input and internally consistent across power and energy representations.
All downstream GPU metrics are derived exclusively from this reconstructed signal, eliminating ambiguity arising from heterogeneous raw telemetry.
The architecture guarantees physical plausibility and maximal energy consistency given available observations, while explicitly acknowledging the modeled nature of the result.
It deliberately trades modeling complexity for improved temporal fidelity and reproducibility, establishing a stable foundation for subsequent attribution and aggregation stages.

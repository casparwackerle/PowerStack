...
\textbf{Metric} & \textbf{Type} & \textbf{Unit} & \textbf{Labels} \\
\midrule
\multicolumn{4}{l}{\textit{Total GPU metrics}} \\
\midrule
\code{gpu\_energy\_mj} & Counter & mJ & \code{gpu\_uuid}, \code{kind}, \code{source} \\
\code{gpu\_power\_mw}  & Gauge   & mW & \code{gpu\_uuid}, \code{kind}, \code{source} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\code{system\_power\_mw}  & Gauge   & mW & \code{chassis}, \code{source}, \code{kind} \\
\code{system\_energy\_mj} & Counter & mJ & \code{chassis}, \code{source}, \code{kind} \\
\bottomrule
\end{tabular}
\caption{Exported Redfish-derived system metrics. The \code{source} label distinguishes \code{redfish\_raw} from \code{redfish\_corrected}.}
...
Metric name & Type & Unit & Labels \\
\midrule
\multicolumn{4}{l}{\textbf{Residual energy and power}} \\
residual\_energy\_mj & Counter & mJ & chassis, source, kind=total \\
residual\_power\_mw  & Gauge   & mW & chassis, source, kind=total \\
\midrule
\multicolumn{4}{l}{\textbf{Residual window validity}} \\
residual\_window\_usable & Gauge & bool & chassis, source \\
\bottomrule
\end{tabular}
\end{table}
...
Metric name & Type & Unit & Labels \\
\midrule
\multicolumn{4}{c}{Idle} \\
\midrule
\code{tycho\_rapl\_energy\_mj} & Counter & mJ &
\code{domain, kind="idle", source} \\
\code{tycho\_rapl\_power\_mw} & Gauge & mW &
\code{domain, kind="idle", source} \\
\midrule
\multicolumn{4}{c}{Dynamic} \\
\midrule
\code{tycho\_rapl\_energy\_mj} & Counter & mJ &
\code{domain, kind="dynamic", source} \\
\code{tycho\_rapl\_power\_mw} & Gauge & mW &
\code{domain, kind="dynamic", source} \\
\bottomrule
\end{tabular}
...
Metric name & Type & Unit & Labels \\
\midrule
\multicolumn{4}{l}{\textbf{Residual power}} \\
residual\_power\_mw & Gauge & mW & chassis, source, kind \\
\midrule
\multicolumn{4}{l}{\textbf{Residual energy}} \\
residual\_energy\_mj & Counter & mJ & chassis, source, kind \\
\midrule
\multicolumn{4}{l}{\textbf{Residual idle model state}} \\
residual\_idle\_baseline\_mw & Gauge & mW & chassis \\
\midrule
\multicolumn{4}{l}{\textbf{Residual window validity}} \\
residual\_window\_usable & Gauge & bool & chassis \\
\bottomrule
\end{tabular}
\end{table}
...
Metric name & Type & Unit & Labels \\
\midrule
\multicolumn{4}{l}{\textit{Power metrics}} \\
gpu\_power\_mw & Gauge & mW & gpu\_uuid, kind $\in\{\text{total},\text{idle},\text{dynamic}\}$, source \\
\midrule
\multicolumn{4}{l}{\textit{Energy metrics}} \\
gpu\_energy\_mj & Counter & mJ & gpu\_uuid, kind $\in\{\text{total},\text{idle},\text{dynamic}\}$, source \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Architectural Context}
This section realizes the workload-level aggregation model defined in \S~\ref{sec:slice16}, mapping per-process eBPF utilization deltas to workload-attributed monotonic counters while enforcing strict conservation and residual \code{\_\_system\_\_} semantics.
...
\paragraph{Exported Metrics}
The exported metrics are summarized in Table~\ref{tab:ebpf-util-exported-metrics}.
...
Metric name & Type & Unit & Labels \\
\midrule
\multicolumn{4}{l}{CPU execution activity} \\
\midrule
\code{tycho\_workload\_bpf\_cpu\_instructions\_total} & Counter & count &
\code{source, namespace, pod, container} \\
\code{tycho\_workload\_bpf\_cpu\_cycles\_total} & Counter & count &
\code{source, namespace, pod, container} \\
\code{tycho\_workload\_bpf\_cache\_misses\_total} & Counter & count &
\code{source, namespace, pod, container} \\
\code{tycho\_workload\_bpf\_page\_cache\_hits\_total} & Counter & count &
\code{source, namespace, pod, container} \\
\midrule
\multicolumn{4}{l}{Interrupt activity} \\
\midrule
\code{tycho\_workload\_bpf\_irq\_net\_tx\_total} & Counter & count &
\code{source, namespace, pod, container} \\
\code{tycho\_workload\_bpf\_irq\_net\_rx\_total} & Counter & count &
\code{source, namespace, pod, container} \\
\code{tycho\_workload\_bpf\_irq\_block\_total} & Counter & count &
\code{source, namespace, pod, container} \\
\midrule
\multicolumn{4}{l}{Process runtime} \\
\midrule
\code{tycho\_workload\_bpf\_process\_run\_us\_total} & Counter & $\mu$s &
\code{source, namespace, pod, container} \\
\bottomrule
\end{tabular}
\end{table}
...
All guarantees stated in \S~\ref{sec:arch_stage4_workload_resolution} are enforced here under discretization, metadata delay, and partial observability.
...
Metric name & Type & Unit & Labels \\
\midrule
\multicolumn{4}{l}{\textbf{CPU Dynamic Energy Attribution}} \\
workload\_rapl\_energy\_mj & Counter & mJ &
domain, kind, source, namespace, pod, container \\
\bottomrule
\end{tabular}
...
\begin{table}[t]
\centering
\caption{Exported CPU idle energy workload metrics}
\label{tab:cpu-idle-attrib-exported-metrics}
\begin{tabular}{llll}
\toprule
Metric name & Type & Unit & Labels \\
\midrule
\multicolumn{4}{l}{\textbf{Workload-attributed CPU idle energy}} \\
\code{tycho\_workload\_rapl\_energy\_mj} & Counter & mJ &
domain, kind{=}idle, source{=}rapl, namespace, pod, container \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{llll}
    ...
    \textbf{Metric} & \textbf{Type} & \textbf{Unit} & \textbf{Labels} \\
    \midrule
    \multicolumn{4}{l}{\textbf{Workload GPU energy}} \\
    \code{tycho\_workload\_gpu\_energy\_mj} & Counter & mJ &
    \code{gpu\_uuid}, \code{kind="dynamic"}, \code{namespace}, \code{pod}, \code{container} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\paragraph{Core Mechanisms}
For each GPU UUID, the absolute corrected idle energy counter value is loaded and written into the corresponding \code{tycho\_workload\_gpu\_energy\_mj} series with \code{kind="idle"} and workload labels set to \code{\_\_system\_\_}.
...
\begin{table}[t]
  ...
  \begin{tabular}{llll}
    ...
    \multicolumn{4}{l}{\textbf{Workload GPU energy}} \\
    \code{tycho\_workload\_gpu\_energy\_mj} & Counter & mJ &
    \code{gpu\_uuid}, \code{kind="idle"}, \code{namespace}, \code{pod}, \code{container} \\
    \bottomrule
  \end{tabular}
\end{table}

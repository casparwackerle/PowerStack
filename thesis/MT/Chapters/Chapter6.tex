...
Together, these two evidence tiers support an evaluation focused on interpretability, consistency, and robustness within limits imposed by the measurement signals.

\section{Qualitative Validation and Consistency Assessment}
\label{sec:qualitative-validation}

\subsection{Invariant-Driven Validation Strategy}
...
Rather than evaluating output values against an external reference, validation focuses on invariants whose violation would render attribution results physically uninterpretable, irrespective of apparent numerical plausibility.

Validation was performed continuously and at feature granularity throughout development.
Each newly introduced mechanism was exercised under adversarial and boundary conditions until its behavior conformed to design intent across relevant operating regimes.
This process relied on sustained inspection of raw input signals, intermediate analysis state, and exported attribution results, allowing violations to surface immediately rather than being inferred post hoc from aggregate outputs.

The validation strategy was explicitly invariant-driven.
Core invariants monitored across all executions included conservation of energy across attribution boundaries, completeness of attribution, monotonicity of cumulative counters, temporal coherence of analysis windows under asynchronous updates, and stability of attribution under repeated, steady, bursty, and concurrent workloads.
These invariants impose global constraints on system behavior; violations propagate across layers and time windows and manifest as observable inconsistencies between independent representations of system state.

No quantitative artifact was produced for this validation tier, as correctness was established through sustained invariant verification rather than post hoc analysis.
This approach establishes internal correctness and semantic coherence of the attribution pipeline, providing the necessary foundation for the targeted experimental analyses presented in the following section.

\subsection{Invariant Witnesses Across the Attribution Pipeline}
\label{sec:validation-layers}

Qualitative validation was applied across the full attribution pipeline, with each major subsystem acting as a witness for specific structural invariants.
Rather than validating components in isolation, the focus was on identifying how violations of metric integrity, temporal coherence, attribution semantics, or observability would necessarily propagate into detectable end-to-end inconsistencies.

\subsubsection{Metric Integrity and Identity Preservation}
\label{sec:validation-ingestion}

Validation of metric ingestion focused on preserving the invariants of monotonicity, continuity, and stable identity mapping across devices and workloads.
Violations at this stage would manifest as counter regressions, unexplained discontinuities, inconsistent unit scaling, or instability in device and workload labeling, all of which would immediately compromise downstream attribution.

All cumulative energy counters exhibited strictly monotonic behavior over sustained execution.
Signal continuity was preserved across steady, bursty, and concurrent workloads, and no silent gaps or unexplained discontinuities were observed.
Device identities and workload labels remained stable across successive observations, preventing cross-device or cross-workload energy leakage.
Independent signal sources were routinely inspected jointly to ensure qualitative plausibility, confirming that activity in one domain did not induce unexplained effects in unrelated components or devices.

System-level power measurements obtained via Redfish were observed to exhibit variable reporting latency relative to CPU and GPU counters.
This behavior was treated as a characterized property of the source rather than an ingestion defect.
Importantly, delayed updates remained coherent and monotonic and did not introduce semantic contradictions that would undermine attribution integrity.

\subsubsection{Temporal Coherence under Asynchronous Signals}
\label{sec:validation-temporal}

Temporal coherence was assessed with a focus on preserving causal ordering and stable window semantics in the presence of heterogeneous sampling rates, asynchronous updates, and source-specific latency.
Correctness depends on coherent window materialization and predictable interpretation of delayed observations during analysis.

Invariant violations at this stage would manifest as negative or oscillatory attribution, unstable residual behavior, or inconsistent transitions between workload phases.
Such effects were actively sought by inspecting intermediate timing state, analysis windows, and derived metrics under controlled transitions between idle, steady, bursty, and concurrent workloads, as well as during long-running executions where accumulated drift or buffer exhaustion would become observable.

System-level power metrics obtained via Redfish exhibited delayed response relative to CPU and GPU component counters.
Validation instead confirmed that delayed samples were incorporated consistently without violating conservation, monotonicity, or attribution completeness.
Observed lag and smoothing effects in derived metrics therefore reflect expected source characteristics and aggregation granularity rather than temporal inconsistencies in the analysis pipeline.

\subsubsection{Attribution Semantics and Energy Conservation}
...
For every analysis window, all observed energy must be accounted for exactly once, either through explicit workload attribution or as transparently exposed residual or system-level energy, within numerical tolerance.
Any silent loss, duplication, or implicit discarding of energy would immediately invalidate interpretability and was therefore treated as a structural failure.

Attribution behavior was inspected under steady, bursty, and concurrent workloads to confirm stable separation and interaction of idle, dynamic, and residual components.
Invariant violations at this stage would manifest as unexplained discrepancies between aggregate and attributed energy or as unstable, sign-inconsistent, or phase-dependent residual behavior.
No such effects were observed across sustained execution.

Boundary cases were explicitly identified and characterized.
In particular, extreme latency of system-level power signals can induce transient distortions in residual attribution during rapid load transitions.
...
\subsubsection{End-to-End Observability and Violation Detectability}
\label{sec:validation-observability}

End-to-end observability served as a primary mechanism for detecting invariant violations throughout development.
Tycho was continuously inspected across multiple abstraction levels, including raw input metrics, intermediate analysis state, and exported attribution outputs, enabling direct cross-checks between independently derived views of system behavior.

Validation focused on a small set of canonical sanity patterns that must hold for any interpretable attribution system.
...
Violations of structural invariants would necessarily manifest as contradictions between these views and were therefore actively sought during exploratory and adversarial testing.
...
The experiments do not serve to verify correctness, which is established through qualitative validation, but to expose observable behavior, strengths, and boundary conditions under concrete workload scenarios.
...
The SUT executed only the monitoring stack, workload pods, and the Tycho exporter, ensuring that observed signals reflect workload and system behavior rather than control-plane activity.
...
An Ansible-based automation workflow materializes the plan, provisions an isolated Kubernetes testing namespace, and orchestrates execution end-to-end without manual intervention, ensuring reproducibility across runs.
...
\begin{enumerate}
  ...
  Duration: 180\,s; workloads: none (sleep only).
  Purpose: idle variance estimation and initial drift detection.

  \item \textbf{CPU warm-up ramp.}
  Workload: \code{stress-ng} CPU (\code{matrixprod}); duration: 60\,s; structure: stepped load increase.
  Purpose: preconditioning and stabilization of subsequent CPU measurements.
  Note: excluded from quantitative analysis.

  \item \textbf{CPU steady baseline (matrix multiplication).}
  Workload: \code{stress-ng} CPU (\code{matrixprod}); duration: 120\,s; CPU request: 16\,000\,mCPU; concurrency: single pod.
  Purpose: steady-state stability and repeatability reference.

  \item \textbf{CPU burst train.}
  Workload: \code{stress-ng} CPU burst (\code{matrixprod}); duration: 180\,s; structure: 9\,s on / 9\,s off; CPU request: 16\,000\,mCPU.
  Purpose: burst attribution fidelity and temporal smearing assessment.

  \item \textbf{CPU jitter train.}
  Workload: \code{stress-ng} CPU jitter (\code{matrixprod}); duration: 180\,s; structure: random gaps between 3\,s and 15\,s; CPU request: 16\,000\,mCPU.
  Purpose: attribution robustness under irregular activity.

  \item \textbf{CPU discrimination (concurrent, heterogeneous pair I).}
  Workloads: \code{stress-ng} \code{int128} and \code{fft}; duration: 180\,s; CPU request: 8\,000\,mCPU per workload; concurrency: two pods.
  Purpose: workload-level dynamic energy differentiation.

  \item \textbf{CPU discrimination (concurrent, heterogeneous pair II).}
  Workloads: \code{stress-ng} \code{bitops} and \code{matrixprod}; duration: 180\,s; CPU request: 8\,000\,mCPU per workload; concurrency: two pods.
  Purpose: discrimination across method-dependent CPU characteristics.

  \item \textbf{CPU idle-allocation fairness (busy vs.\ noop).}
  Workloads: active \code{stress-ng} CPU (\code{bitops}) and idle sleep pod; duration: 180\,s; CPU request: 8\,000\,mCPU per workload.
  Purpose: idle energy sharing semantics under co-scheduling.

  \item \textbf{GPU idle baseline.}
  Duration: 120\,s; workloads: none (GPU idle).
  Purpose: GPU idle stability and cross-domain isolation.

  \item \textbf{GPU steady baseline.}
  Workload: steady GPU burn; duration: 180\,s; GPU request: one device.
  Purpose: steady-state GPU attribution stability and repeatability.

  \item \textbf{GPU burst train.}
  Workload: GPU burst burn; duration: 240\,s; structure: 3\,s on / 3\,s off; GPU request: one device.
  Purpose: temporal aggregation limits for GPU attribution.

  \item \textbf{GPU concurrency (two workloads).}
  Workloads: two identical steady GPU burns; duration: 180\,s; GPU request: one device per workload.
  Purpose: two-way proportional GPU energy splitting.

  \item \textbf{GPU concurrency (three workloads).}
  Workloads: three identical steady GPU burns; duration: 180\,s; GPU request: one device per workload.
  Purpose: scaling and slicing robustness under higher concurrency.

  \item \textbf{Idle baseline (end).}
  Duration: 180\,s; workloads: none (sleep only).
  Purpose: post-run drift and stability verification.
\end{enumerate}
...
In contrast, idle-attributed components exhibit significantly lower variability and remain stable across repetitions.
...
Figures~\ref{fig:cpu-idle-fairness-idle} and~\ref{fig:cpu-idle-fairness-busy} summarize the observed workload-level CPU package-domain power attributed to an active and a noop pod under idle and busy system conditions, respectively.
...
The results indicate that Tycho allocates idle CPU energy strictly based on declared resources, while dynamic energy attribution remains sensitive to effective execution and contention.
...
\paragraph{Observed Behavior.}
Figures~\ref{fig:cpu-discrimination-heterogeneous-idle} and~\ref{fig:cpu-discrimination-heterogeneous-busy} summarize the observed workload-level \emph{dynamic} CPU core-domain power attributed to four heterogeneous workloads under idle and busy system conditions, respectively.
...
Across repetitions on the idle system, each workload exhibits a clearly distinguishable dynamic power profile as reflected in the distributions shown above.
The relative ordering between workload types remains stable, with \code{BitOps} consistently consuming the highest dynamic power, followed by \code{FFT} and \code{int128}, while \code{MatrixProd} exhibits the lowest power consumption.
...
Simple integer workloads such as \code{BitOps} sustain high instruction retirement rates with minimal dependency on memory or complex control flow, resulting in high dynamic power.
...
To isolate whether these differences are intrinsic to the workloads or predominantly driven by instruction volume, the same experiment is analyzed with dynamic energy normalized by the number of retired instructions.
...
For idle execution, \code{FFT} and \code{int128} exhibit consistently higher energy per instruction than \code{BitOps} and \code{MatrixProd}, despite comparable instruction counts.

A plausible explanation lies in differences in instruction mix and microarchitectural utilization.
\code{FFT} and \code{int128} workloads activate more energy-intensive execution units and backend activity per retired instruction than simple integer operations.
In contrast, \code{BitOps} primarily exercise lightweight integer pipelines, while \code{MatrixProd} is more strongly affected by memory access patterns that limit sustained execution-unit utilization.
These differences are most clearly expressed on the idle system, where higher operating frequencies and reduced contention expose workload-dependent microarchitectural behavior.
...
\paragraph{Observed Behavior.}
Figure~\ref{fig:gpu-concurrent-discrimination} summarizes the observed workload-level GPU power attributed to two concurrent workloads, each bound to a distinct physical GPU.
...
\paragraph{Observed Behavior.}
Figure~\ref{fig:gpu-concurrent-oversubscription} summarizes the observed workload-level GPU power attributed to three concurrent workloads executing on two physical GPUs, resulting in one single-workload and one two-workload device configuration.
...
\paragraph{Observed Behavior.}
Figure~\ref{fig:sudden-load-transitions} shows two representative time series from individual executions, illustrating characteristic transient behavior rather than averaged trends during a sudden onset of CPU and GPU stress, respectively.
...
During this interval, Tycho attributes the rapidly increasing component power before the delayed system power increase is fully observed, resulting in a transient residual imbalance.

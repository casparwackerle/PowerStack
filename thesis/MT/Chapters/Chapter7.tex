...
% PURPOSE:
% - Establish a shared mental model before answering RQs
% - Summarize recurring patterns, not individual tests
% - No figures, no test descriptions, no references to specific plots

Across the evaluated scenarios, several consistent patterns emerged that
characterize Tycho’s behavior as a measurement system.

% PLACEHOLDER:
% Summarize high-level observations such as:
% - Stability and repeatability under steady workloads
% - Robust localization of dynamic energy under bursty execution
% - Consistent attribution under concurrent workloads
% - Conservation properties across attribution boundaries
% - External telemetry latency as the dominant limiting factor

These observations form the empirical basis for the following evaluation and
are not tied to individual experiments, but to recurring system-level behavior
across the experimental space explored in this thesis.

% ----------------------------------------------------------------------
 \section{Research Question 1: Guarantees and Feasibility of Accuracy-First Measurement}
\label{sec:eval-rq1}
...
\begin{quote}
An accuracy-first energy measurement system can realistically guarantee
...
\section{Research Question 2: Interpretability and Explanatory Power of Semantics-Driven Attribution}
\label{sec:eval-rq2}
...
\begin{quote}
An explicit, semantics-driven attribution methodology increases the
...
% ----------------------------------------------------------------------
\section{Research Question 3: Contexts and Trade-offs of Accuracy-First Energy Measurement}
\label{sec:eval-rq3}
...
As a consequence, Tycho cannot compensate for missing or incorrect telemetry.
Rather than masking data limitations through interpolation or heuristic
completion, the system exposes their effects directly in the attribution
results. This positioning emphasizes transparency and bounded interpretation
over completeness.

...
A central component of Kepler's data collection is an eBPF program that attaches a \code{kprobe} to the kernel function \code{finish\_task\_switch}. This function is triggered during every context switch and receives a pointer to the \code{task\_struct} representing the task that is being descheduled. Kepler uses this hook to measure how long a task was running on the CPU and to attribute performance counter deltas to that task.

To track on-CPU runtime, Kepler maintains a timestamp map keyed by \code{<pid, cpu>} that records the last time a process was scheduled on a specific core. When a context switch occurs, the elapsed time is used to increment the \code{process\_run\_time} metrics for the outgoing process. This provides fine-grained accounting of CPU residency per process. Additional kprobes are used to monitor page cache access (\code{mark\_page\_accessed}) and dirtying activity (\code{set\_page\_dirty}), enabling the tracking of page cache hits as a proxy for memory locality and behavior.
...
\paragraph{Hardware Counters via \code{perf\_event\_open} and eBPF}
...
Kepler uses Intel RAPL (Running Average Power Limit) to obtain high-resolution energy consumption data from the CPU package. It prefers reading from the Linux \code{powercap} sysfs interface, but falls back to using MSRs or hardware monitor drivers on platforms where this is not available.

The main RAPL domains used are:
\begin{itemize}
  \item \code{pkg}: Total package power (including cores, caches, and memory controller)
  \item \code{dram}: DRAM energy (if supported)
  \item \code{pp0}: Cores only (used less frequently)
\end{itemize}

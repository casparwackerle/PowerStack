...
Scaphandre attributes power consumption to processes and containers using a straightforward proportional model based on CPU usage over time. At each sampling interval, Scaphandre calculates the total energy consumption of the host system using RAPL counters. This total energy delta is then divided among all active processes according to their normalized CPU utilization. Time spent in non-active states (such as \texttt{iowait}, \texttt{idle}, \texttt{irq}, and \texttt{softirq}) is \textit{not} included in the total CPU time considered during this interval, effectively filtering out idle CPU time. The attribution formula, as conceptually implemented in \texttt{get\_process\_power\_consumption\_microwatts()}, computes per-process energy as follows:
\begin{equation}
E_{\text{proc}}(t) = \frac{E_{\text{RAPL}}(t) \cdot \text{CPU}_{\text{proc}}(t)}{\sum_i \text{CPU}_i(t)}
\end{equation}
Here, \( E_{\text{RAPL}}(t) \) is the energy consumed during the interval (typically from the RAPL PKG or PSys domain), and \( \text{CPU}_{\text{proc}}(t) \) denotes the process’s CPU usage normalized over all available cores. This normalization is computed by dividing the raw process CPU usage by the total number of CPU cores. The model assumes that energy usage is driven primarily by CPU activity, and distributes all observed energy accordingly.

Container-level attribution is achieved by inspecting the cgroup paths of each process. If a process can be mapped to a container or pod (via Docker or Kubernetes APIs), the corresponding metadata labels are attached to the exported metrics. These labels include \texttt{container\_id}, \texttt{kubernetes\_pod\_name}, and \texttt{namespace}. The key metric \texttt{scaph\_process\_power\_consumption\_microwatts} is thus enriched with container metadata, enabling aggregation at the container or pod level (e.g.s in \textit{Prometheus}).

Scaphandre also exposes host-level energy metrics, such as \texttt{scaph\_host\_power\_microwatts} and \texttt{scaph\_host\_energy\_microjoules}, typically derived from the RAPL PSys domain if available, or from PKG and DRAM domains otherwise. Unlike process-level metrics, host-level energy calculations do \textit{not} exclude idle CPU time.

While this attribution logic is computationally lightweight and easy to interpret, it comes with certain limitations. Since only CPU usage is considered, power consumed by memory, storage, networking, and other subsystems is not explicitly modeled. Furthermore, all observed energy is attributed to active processes; baseline idle consumption, power supply losses, or OS-level overhead (e.g., context switching) are not separately accounted for. This may introduce inaccuracies in low-utilization scenarios or for I/O-bound workloads.

\paragraph{Idle Consumption}
Scaphandre does not directly measure CPU idle energy consumption. Inactive CPU time (such as \texttt{iowait}, \texttt{idle}, \texttt{irq}, and \texttt{softirq}) is explicitly excluded from the CPU time considered in process attribution. However, an indirect estimate of idle power can be obtained using host-level energy metrics. The residual energy
\[
P_{\text{idle}} = P_{\text{Host}} - \sum P_{\text{proc}}
\]
can be interpreted as "power due to idle activity". Since \( P_{\text{Host}} \) may stem from different RAPL domains depending on hardware availability and sensor configuration, this residual estimate provides only limited insight into true idle power. Ambiguity in RAPL domain coverage further reduces its precision.
...
The developers acknowledge the lack of public documentation for RAPL, particularly regarding the \texttt{PSys} domain, which they assume to reflect total SoC power. Their experiments also raise the unresolved question of whether the \texttt{DRAM} domain is included within \texttt{PKG}, or whether it must be treated independently.

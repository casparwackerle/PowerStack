...
\begin{align}
    ...
    E_{\text{process}} = \; &
    \sum_{\text{comp} \in \{\text{pkg}, \text{core}, \text{dram}, \text{uncore}, \text{gpu}\}} 
    \left(
    \left\lceil 
...
This timing issue primarily affects metrics that are reported as cumulative deltas (e.g., energy in millijoules). For Redfish-based metrics, which are provided in watts and converted internally by Kepler into energy values by multiplying with the update interval, synchronization is less problematic. The conversion to energy is performed over a well-defined window, which inherently matches the Prometheus export interval, thus avoiding the same class of timing mismatches.

...
Kepler collects a broad set of low-level metrics that form the foundation for power attribution. These include hardware performance counters such as CPU instructions, CPU cycles, reference cycles, and cache misses. In addition, eBPF-based probes are used to collect CPU time, block I/O, and network IRQ activity. All metrics are exposed as Prometheus counters, allowing power consumption to be computed as a rate of energy (Joules) over time using standard PromQL expressions (e.g., \code{irate()}).

notes

You need to write an introduction text, very short: Needs to contains:
- eBPF metric: Kepler traps into the \code{finish_task_switch} kernel function (which is responsible for task cleanup) using a bpf textit{kprobe}, which is called before \code{finish_task_switch} is called (as opposed to a kretprobe, which would be called after the probed functionreturns.) The kProbe is "int kprobe__finish_task_switch(struct pt_regs *ctx, struct task_struct *prev)".

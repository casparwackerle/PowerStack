...
Kepler 0.10.0 represents a ground-up redesign focusing on modularity, thread-safety, and accessibility. Mutex locks are used extensively throughout the codebase to ensure safe concurrent metric collection. Unlike previous versions, Kepler now requires only readonly access to \texttt{/proc} and \texttt{/sys} filesystems, eliminating the need for elevated privileges (\texttt{CAP\_SYSADMIN}, \texttt{CAP\_BPF}). This marks a significant improvement in deployability and security.

Container detection still relies on traditional cgroup path analysis, with per-process cgroups parsed via regex matching to identify container runtimes and IDs. However, unlike prior versions, container-level resource metrics are no longer derived by aggregating per-process metrics. Instead, Kepler 0.10.0 treats containers as first-class workloads: CPU usage statistics for containers are read directly from cgroups, without requiring process-level aggregation.

Node-level CPU activity is determined via the Linux \texttt{/proc/stat} interface, using the formula:
...
This represents a significantly simplified utilization model compared to prior versions, which relied on more granular hardware performance counters (e.g., instructions retired, cache misses).

At the power measurement layer, Kepler continues to rely on RAPL readings, now referred to as ``energy zones''. In addition to standard \texttt{package}, \texttt{core}, \texttt{uncore}, and \texttt{dram} zones, Kepler 0.10.0 dynamically detects the \texttt{psys} domain, expanding applicability to non-server-grade processors. Multi-socket systems are supported via automatic aggregation of zones with identical names.
...
The energy attribution model has undergone a substantial simplification. Kepler 0.10.0 attributes energy solely based on CPU time. The node's total RAPL-reported active energy is distributed proportionally to workload CPU activity:
...
\begin{itemize}
    ...
    \item \textbf{Process- and container-level attribution:} Containers and processes are treated as independent workloads. Each receives a fraction of the node's active energy based on its CPU time share:
    \begin{equation}
...
findings so far:
General changes:
- Full architecture rewerite, modernized code, service-oriented, thread-safe.
- Sigificantly reduced security priviledges (no CAP_SYSADMIN ro CAP_BPF)
- Focus exclusively on CPU power via RAPL
- GPU, Memory, disk, network remain unsupported
- documentation minimal, likely auto-generated

Power attribution:
- idle/dynamic split removed at workload level
--> total container, pod, VM and process energy now reflects ONLY ACTIVE cpu usage (like in Pijnacker)
--> Node-level still exports both idle and active energy
New attribution model
- strictly usage-based (CPU TIME NOOO)
- potential fixes???

completed/terminated workloads:
- introduced tracking mechanisms:
--> monitor.max-terminated limits number of terminated workloads tracked
--> minTerminatedEnergyThreshold filters low2-energy workloads

Sampling interval
- 5 secodns default

RAPL
- dynamic RAPL zone detection (no more hardcoded detection)

Architecture
- YAML configuration enables modular deployment


MOST PRESSING ISSUES:
- new metrics sources only /proc???
- idle power attribution
--> validate distribution
- completed pods handling
--> review termination handling code
- Sampling interval


Comments:
- many functionalities were removed, notably eBPF metrics. some of these metrics were great ideas and a central selling point of the old kepler version. So far, it is still unclear it these will be re-added in the new architecture, or if this consitutes an entirely new direction. in other words: has Kepler 0.10.0 only introduced a new framework architecture, or is this a major deviation from the original product, more in favor of lightweigh but less accurate measurement.

Code analysis:
- mutex locks ensure thread safety
- only /sys and /proc are needed
- cgroup is still used to get infos about the container a process is running in
- /proc/pid/stat is used via procfs to extract CPU statistics.
- IMPORTANT: (implemented in CPUUsaatgeRatio): the cpu usage time (node-level) is calculated across the node: activeTime = total time - (idle + iowait). In addition, a ration \frac{active time}{total time} is calculated. This model is much simpler when compared to the previous kepler version, and the use of CPUtime for attribution is a questioable move (see ~\ref{sec:cpu-metric-challenges})
- in addition to the rapl domains (called 'zones' in the new codebase) pkg, core, uncore and DRAM, kepler now also looks for Psys, making this more useful for non-server-grade machines. Zones are dynamically detected, and can be filtered by user configuration.
- for multi-socket devices, zones are grouped.
- 8s pod lifecycle awareness has significantly improved. (please insert some insights based on the codebase we analyzed, such as the handling of ephemeral/init containers)

Power attribution model:
- Node-level: activeEnergy = deltaEnergy * nodeCPUUsageRatio and idleEnergy = deltaEnergy - activeEnergy
- conainer-level: Each process gets a share of the node’s total active energy, proportional to its CPU time relative to total node CPU time.
- Kepler now treats containers as first-class workloads
